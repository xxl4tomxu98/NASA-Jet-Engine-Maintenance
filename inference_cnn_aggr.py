...
def load_part_array(sample_dir_path, unit_num,
    ...
    filename = 'Unit%s_win%s_str%s_part%s.npz' % (str(int(unit_num)), win_len,
                                                  stride, part_num)
...
def load_part_array_merge(sample_dir_path, unit_num, win_len,
                          win_stride, partition):
    sample_array_lst = []
    label_array_lst = []
    print("Unit: ", unit_num)
    for part in range(partition):
        print("Part.", part+1)
        sample_array, label_array = load_part_array(sample_dir_path, unit_num,
                                                    win_len, win_stride,
                                                    part+1)
...
def load_array(sample_dir_path, unit_num, win_len, stride, sampling):
    filename = 'Unit%s_win%s_str%s_smp%s.npz' % (str(int(unit_num)), win_len,
                                                 stride, sampling)
    filepath = os.path.join(sample_dir_path, filename)
    loaded = np.load(filepath)
...
def figsave(history, win_len, win_stride, bs, lr, sub):
    ...
    fig_acc.savefig(pic_dir + "/training_w%s_s%s_bs%s_sub%s_lr%s.png"
...
def get_flops(model):
    ...
    frozen_func, graph_def = \
        convert_variables_to_constants_v2_as_graph(concrete_func)
...
def main():
    ...
    parser.add_argument('--sampling', type=int, default=1,
                        ...
                        then this indicates that we assumes 0.1Hz of data \
                        collection')
    ...
    for index in units_index_train:
        ...
        sample_array, label_array = load_array(sample_dir_path, index,
                                               win_len, win_stride, sampling)
    ...
    # input_temp = Input(shape=(sample_array.shape[1], sample_array.shape[2]),
    #                    name='kernel_size%s' %str(int(kernel_size)))
    # #------
    # one_d_cnn = one_dcnn(n_filters, kernel_size, sample_array, initializer)
    # cnn_out = one_d_cnn(input_temp)
    # x = cnn_out
    # # x = Dropout(0.5)(x)
    # main_output = Dense(1, activation='linear',
    ...
    one_d_cnn_model = one_dcnn(n_filters, kernel_size,
                               sample_array, initializer)
    print(one_d_cnn_model.summary())
    # one_d_cnn_model.compile(loss='mean_squared_error',
    #                         optimizer=amsgrad, metrics=[rmse, 'mae'])

    start = time.time()
    # lr_scheduler = LearningRateScheduler(scheduler)
    one_d_cnn_model.compile(loss='mean_squared_error',
                            optimizer=amsgrad, metrics='mae')
    history = one_d_cnn_model.fit(sample_array, label_array, epochs=ep,
                                  batch_size=bs, validation_split=vs,
                                  verbose=2,
                                  callbacks=[
                                    EarlyStopping(monitor='val_loss',
                                                  min_delta=0, patience=pt,
                                                  verbose=1, mode='min'),
                                    ModelCheckpoint(model_temp_path,
                                                    monitor='val_loss',
                                                    save_best_only=True,
                                                    mode='min', verbose=1)]
                                  )
    ...
    for idx in range(len(units_index_test)):
        ...
        plt.title('Unit%s inference' % str(int(units_index_test[idx])),
                  fontsize=30)
        ...
        plt.show()
        fig_verify.savefig(pic_dir +
                           "/unit%s_test_w%s_s%s_bs%s_lr%s_sub%s_rmse-%s.png"
