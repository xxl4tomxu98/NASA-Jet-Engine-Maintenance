...
def df_all_creator(data_filepath):
    ...
    '''
    Illusration of Multivariate time-series of condition monitoring
    sensors readings for Unit5 (fifth engine)
    ...
    # df_all = pd.concat([df_W, df_Xs, df_Xv, df_Y, df_A],
    #                    axis=1).drop(columns)
...
def gen_sequence(id_df, seq_length, seq_cols):
    ...
    we need to drop those which are below the window-length. An alternative
    would be to pad sequences so that we can use shorter ones """
...
def gen_labels(id_df, seq_length, label):
    """ Only sequences that meet the window-length are considered,
    no padding is used. This means for testing dropping those which
    are below the window-length. An alternative would be to pad
    sequences so that we can use shorter ones """
...
def time_window_slicing_label_save(input_array, sequence_length,
                                   stride, index, sample_dir_path,
                                   sequence_cols='RUL'):
    '''
    ref
    ...
    input_temp = input_array[input_array['unit'] == index]
    input_temp = input_temp[sequence_cols].values
    ...
    # np.save(os.path.join(sample_dir_path,
    #                      'Unit%s_rul_win%s_str%s' %(str(int(index)),
...
def time_window_slicing_sample_save(input_array, sequence_length, stride,
    ...
    # np.save(os.path.join(sample_dir_path,
    #                      'Unit%s_samples_win%s_str%s' % (str(int(index)),
    #                      sequence_length, stride)),
...
class Input_Gen(object):
    ...
    def __init__(self, df_train, df_test, cols_normalize, sequence_length,
                 sequence_cols, sample_dir_path, unit_index, stride):
    ...
    def seq_gen(self):
        ...
        :return:
        '''
        if any(index == self.unit_index for
               index in self.df_train['unit'].unique()):
            print("Unit for Train")
            label_array = time_window_slicing_label_save(self.df_train,
        ...
        np.savez_compressed(os.path.join(
            self.sample_dir_path,
            'Unit%s_win%s_str%s_part1' % (str(int(self.unit_index)),
                                          self.sequence_length, self.stride)),
                            sample=sample_split_lst[0],
                            label=label_split_lst[0])
        print("part1 saved")
        np.savez_compressed(os.path.join(
                self.sample_dir_path,
                'Unit%s_win%s_str%s_part2' % (str(int(self.unit_index)),
                                              self.sequence_length,
                                              self.stride)),
                            sample=sample_split_lst[1],
                            label=label_split_lst[1])
        print("part2 saved")
        np.savez_compressed(os.path.join(
            self.sample_dir_path,
            'Unit%s_win%s_str%s_part3' % (str(int(self.unit_index)),
                                          self.sequence_length,
                                          self.stride)),
                            sample=sample_split_lst[2],
                            label=label_split_lst[2])
        print("part3 saved")
        return
