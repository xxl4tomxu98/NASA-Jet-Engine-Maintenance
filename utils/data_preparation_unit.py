...
def df_all_creator(data_filepath, sampling):
    ...
    '''
    Illusration of Multivariate time-series of condition monitoring sensors
    readings for Unit5 (fifth engine)
    W: operative conditions (Scenario descriptors) - ['alt', 'Mach', 'TRA',
                                                      'T2']
    X_s: measured signals - ['T24', 'T30', 'T48', 'T50', 'P15', 'P2', 'P21',
                             'P24', 'Ps30', 'P40', 'P50', 'Nf', 'Nc', 'Wf']
    X_v: virtual sensors - ['T40', 'P30', 'P45', 'W21', 'W22', 'W25', 'W31',
                            'W32', 'W48', 'W50', 'SmFan', 'SmLPC', 'SmHPC',
                            'phi']
    T(theta): engine health parameters - ['fan_eff_mod', 'fan_flow_mod',
    ...
    # df_all = pd.concat([df_W, df_Xs, df_Xv, df_Y, df_A], axis=1).drop
    # (columns=['P45', 'W21', 'W22', 'W25', 'W31', 'W32', 'W48', 'W50',
    #  'SmFan', 'SmLPC', 'SmHPC', 'phi', 'Fc', 'hs'])
...
def gen_sequence(id_df, seq_length, seq_cols):
    """ Only sequences that meet the window-length are considered, no padding
    is used. This means for testing. we need to drop those which are below
    the window-length. An alternative would be to pad sequences so that
...
def gen_labels(id_df, seq_length, label):
    """ Only sequences that meet the window-length are considered, no padding
    is used. This means for testing we need to drop those which are below the
    window-length. An alternative would be to pad sequences so that
    we can use shorter ones """
...
def time_window_slicing(input_array, sequence_length, sequence_cols):
    # generate labels
    label_gen = [gen_labels(input_array[input_array['unit'] == id],
                            sequence_length, ['RUL'])
                 for id in input_array['unit'].unique()]
    label_array = np.concatenate(label_gen).astype(np.float32)
    # label_array = np.concatenate(label_gen)
    # transform each id of the train dataset in a sequence
    seq_gen = (list(gen_sequence(input_array[input_array['unit'] == id],
...
def time_window_slicing_label_save(input_array, sequence_length, stride,
                                   index, sample_dir_path,
                                   sequence_cols='RUL'):
    '''
    ref
    ...
    input_temp = (input_array[input_array['unit'] ==
                  index][sequence_cols].values)
    ...
    # np.save(os.path.join(sample_dir_path, 'Unit%s_rul_win%s_str%s'
    #                      %(str(int(index)), sequence_length, stride)),
    #         label_array)  # save the file as "outfile_name.npy"
    return label_array[:, -1]


def time_window_slicing_sample_save(input_array, sequence_length, stride,
                                    index, sample_dir_path, sequence_cols):
    # generate labels
    window_lst = []  # a python list to hold the windows
    input_temp = (input_array[input_array['unit'] ==
                  index][sequence_cols].values)
    ...
    # np.save(os.path.join(sample_dir_path, 'Unit%s_samples_win%s_str%s
    #         ' %(str(int(index)), sequence_length, stride)),
...
class Input_Gen(object):
    ...
    '''
    def __init__(self, df_train, df_test, cols_normalize, sequence_length,
                 sequence_cols, sample_dir_path, unit_index, sampling, stride):
        # self.__logger = logging.getLogger('data preparation for using
        #                                    it as the network input')
        print("the number of input signals: ", len(cols_normalize))
        min_max_scaler = preprocessing.MinMaxScaler(feature_range=(-1, 1))
        norm_df = pd.DataFrame(min_max_scaler
                               .fit_transform(df_train[cols_normalize]),
                               columns=cols_normalize,
                               index=df_train.index)
        join_df = df_train[df_train.columns
                           .difference(cols_normalize)].join(norm_df)
        df_train = join_df.reindex(columns=df_train.columns)
        norm_test_df = pd.DataFrame(min_max_scaler
                                    .transform(df_test[cols_normalize]),
                                    columns=cols_normalize,
                                    index=df_test.index)
        test_join_df = df_test[df_test.columns
    ...
    def seq_gen(self):
        ...
        :return:
        '''
        if any(index == self.unit_index for index
               in self.df_train['unit'].unique()):
            print("Unit for Train")
            label_array = time_window_slicing_label_save(
                    self.df_train,
                    ...
                    sequence_cols='RUL')
            sample_array = time_window_slicing_sample_save(
                    self.df_train,
                    self.sequence_length,
                    self.stride,
                    self.unit_index,
                    self.sample_dir_path,
                    sequence_cols=self.cols_normalize)
        else:
            print("Unit for Test")
            label_array = time_window_slicing_label_save(self.df_test,
                                                         self.sequence_length,
                                                         self.stride,
                                                         self.unit_index,
                                                         self.sample_dir_path,
        ...
        np.savez_compressed(os.path.join(self.sample_dir_path,
                                         'Unit%s_win%s_str%s_smp%s'
                                         % (str(int(self.unit_index)),
